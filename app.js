...
class StorageManager {  
  ...
  static exportData() {  
    ...
    a.href = url;  
    a.download = `fuelcalc-backup-${new Date().toISOString().split('T')[0]}.json`;  
    ...
    return new Promise((resolve, reject) => {  
      ...
      reader.onload = (e) => {  
        try {  
          ...
          NotificationManager.showError('Erro ao importar dados: arquivo inválido');  
          reject(error);  
        }  
      };  
...
class Validator {  
  static validateVehicle(nome, eficiencia, tipo) {  
    ...
    if (!SecurityUtils.validateNumber(eficiencia, CONFIG.VALIDATION.MIN_EFFICIENCY, CONFIG.VALIDATION.MAX_EFFICIENCY)) {  
      errors.push(`Eficiência deve estar entre ${CONFIG.VALIDATION.MIN_EFFICIENCY} e ${CONFIG.VALIDATION.MAX_EFFICIENCY} km/L`);  
    }  
  ...
  static validateTrip(kmInicial, kmFinal, kmPorLitro, precoCombustivel, ganhoUber) {  
    const errors = [];  
  
    if (!SecurityUtils.validateNumber(kmInicial, CONFIG.VALIDATION.MIN_KM, CONFIG.VALIDATION.MAX_KM)) {  
      errors.push('KM Inicial deve ser um número válido');  
    }  
  
    if (!SecurityUtils.validateNumber(kmFinal, kmInicial + 0.1, CONFIG.VALIDATION.MAX_KM)) {  
    ...
    if (!SecurityUtils.validateNumber(kmPorLitro, CONFIG.VALIDATION.MIN_EFFICIENCY, CONFIG.VALIDATION.MAX_EFFICIENCY)) {  
      errors.push(`Eficiência deve estar entre ${CONFIG.VALIDATION.MIN_EFFICIENCY} e ${CONFIG.VALIDATION.MAX_EFFICIENCY} km/L`);  
    }  
  
    if (!SecurityUtils.validateNumber(precoCombustivel, CONFIG.VALIDATION.MIN_PRICE, CONFIG.VALIDATION.MAX_PRICE)) {  
      errors.push(`Preço deve estar entre R$ ${CONFIG.VALIDATION.MIN_PRICE} e R$ ${CONFIG.VALIDATION.MAX_PRICE}`);  
    }  
  
    if (ganhoUber !== null && ganhoUber !== '' && !SecurityUtils.validateNumber(ganhoUber, 0, 10000)) {  
...
class VehicleManager {  
  ...
  initDOMCache() {  
    return {  
      ...
      typeButtons: document.querySelectorAll('.vehicle-type-buttons .uber-button')  
    };  
  }  
  ...
  bindEvents() {  
    ...
    if (cancelBtn) cancelBtn.addEventListener('click', () => this.hideVehicleForm());  
  }  
  ...
  saveVehicle() {  
    ...
    const eficiencia = parseFloat(SecurityUtils.convertCommaToPoint(this.domCache.vehicleEfficiency.value));  
    ...
    const existingVehicle = vehicles.find(v =>   
      v.nome.toLowerCase() === nome.toLowerCase() && v.tipo === tipo  
    );  
  ...
  loadVehicles() {  
    const vehicles = StorageManager.safeGetItem(CONFIG.STORAGE_KEYS.VEHICLES);  
    const filteredVehicles = vehicles.filter(v => v.tipo === this.currentType);  
  ...
  createVehicleCard(vehicle) {  
    ...
    card.className = `vehicle-card ${vehicle.id === this.currentVehicle?.id ? 'active' : ''}`;  
  ...
  clearVehicleForm() {  
    ...
    this.domCache.vehicleType.value = 'carro';  
  }  
}  
...
class FuelCalculator {  
  ...
  calculate() {  
    const kmInicial = parseFloat(SecurityUtils.convertCommaToPoint(this.domCache.kmInicial.value));  
    const kmFinal = parseFloat(SecurityUtils.convertCommaToPoint(this.domCache.kmFinal.value));  
    const kmPorLitro = parseFloat(SecurityUtils.convertCommaToPoint(this.domCache.kmPorLitro.value));  
    const precoCombustivel = parseFloat(SecurityUtils.convertCommaToPoint(this.domCache.precoCombustivel.value));  
    const ganhoUber = this.domCache.ganhoUber.value ?   
      parseFloat(SecurityUtils.convertCommaToPoint(this.domCache.ganhoUber.value)) : null;  
  
    const validation = Validator.validateTrip(kmInicial, kmFinal, kmPorLitro, precoCombustivel, ganhoUber);  
    ...
    const results = this.performCalculation(kmInicial, kmFinal, kmPorLitro, precoCombustivel, ganhoUber);  
  ...
  performCalculation(kmInicial, kmFinal, kmPorLitro, precoCombustivel, ganhoUber) {  
  ...
  displayResults(results) {  
    ...
    if (resultElements.distancia) {  
      resultElements.distancia.textContent = `${results.distancia.toFixed(1)} km`;  
    }  
...
class HistoryManager {  
  ...
  bindEvents() {  
    if (this.domCache.verMaisBtn) {  
      this.domCache.verMaisBtn.addEventListener('click', () => this.showFullHistory());  
    }  
      
    if (this.domCache.minimizarBtn) {  
      this.domCache.minimizarBtn.addEventListener('click', () => this.minimizeHistory());  
    }  
      
    if (this.domCache.limparHistoricoBtn) {  
      this.domCache.limparHistoricoBtn.addEventListener('click', () => this.clearHistory());  
    }  
      
    if (this.domCache.fecharModalBtn) {  
      this.domCache.fecharModalBtn.addEventListener('click', () => this.closeModal());  
    }  
  ...
  updateHistory() {  
    const history = StorageManager.safeGetItem(CONFIG.STORAGE_KEYS.HISTORY);  
    const filteredHistory = history.filter(item => item.tipo === this.vehicleManager.currentType);  
  ...
  renderHistoryItems(items, fullHistory) {  
    ...
    items.forEach((item, index) => {  
      ...
      // Event listeners seguros  
      const showDetailsHandler = () => this.showDetails(fullHistory.findIndex(h => h.data === item.data));  
  ...
  showDetails(index) {  
    ...
    const details = [  
      ...
      { label: 'Preço por Litro', value: `R$ ${parseFloat(record.preco).toFixed(2)}` },  
  ...
  clearHistory() {  
    if (confirm(`Tem certeza que deseja limpar todo o histórico de ${this.vehicleManager.currentType}s?`)) {  
      const history = StorageManager.safeGetItem(CONFIG.STORAGE_KEYS.HISTORY);  
      const filteredHistory = history.filter(item => item.tipo !== this.vehicleManager.currentType);  
        
  if (StorageManager.safeSetItem(CONFIG.STORAGE_KEYS.HISTORY, filteredHistory)) {  
        this.updateHistory();  
        if (window.statisticsManager) {  
          window.statisticsManager.updateStatistics();  
        }  
...
class StatisticsManager {  
  constructor(vehicleManager) {  
    ...
    this.domCache = this.initDOMCache();  
    this.chart = null;  
    this.debouncedUpdate = SecurityUtils.debounce(() => this.updateStatistics(), CONFIG.DEBOUNCE_DELAY);  
  }  
  ...
  calculateStatistics(history) {  
    const totalKm = history.reduce((sum, item) => sum + parseFloat(item.distancia), 0);  
    const totalGasto = history.reduce((sum, item) => sum + parseFloat(item.custo), 0);  
    const totalLitros = history.reduce((sum, item) => sum + parseFloat(item.litros), 0);  
...
class AppManager {  
  ...
  bindGlobalEvents() {  
    ...
    // Event listener para tela de carregamento  
...
document.addEventListener('DOMContentLoaded', () => {  
  try {  
    ...
    NotificationManager.showError('Erro ao inicializar aplicação. Recarregue a página.');  
  }  
});  
